...
/**
 * Canonicalize an arbitrary JSON-serializable value for deterministic hashing/signing.
 *
 * Rules:
 ...
 *
 * This is the single source of truth used by the receipt generator, verify-sdk,
 * and all backend routes that sign or verify trust receipts.
 */
export function canonicalizeReceipt(obj: any): string {
  if (obj === null || typeof obj !== 'object') {
    return JSON.stringify(obj);
  }
  if (Array.isArray(obj)) {
    return '[' + obj.map(canonicalizeReceipt).join(',') + ']';
  }
  const sortedKeys = Object.keys(obj).sort();
  const pairs = sortedKeys
    .filter((key) => obj[key] !== undefined)
    .map((key) => JSON.stringify(key) + ':' + canonicalizeReceipt(obj[key]));
  return '{' + pairs.join(',') + '}';
}
