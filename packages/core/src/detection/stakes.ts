...
const HIGH_STAKES_EMBEDDINGS = {
  ...
  investor: embed(
    'investor investors investment portfolio shareholder quarterly quarter earnings benchmark fund fiduciary securities performance disclosure guidance underperformance underperform market volatility long-term strategy communication allocators limited partners'
  ),
  employment: embed(
    'hiring recruitment recruit candidate interview employment workplace discrimination cultural fit team fit personality performance review promotion termination manager workplace'
  ),
...
export function classifyStakes(text: string): StakesEvidence {
  ...
  // DECISION LOGIC
  const investorSignal = entities.types.includes('investor');
  const employmentSignal = entities.types.includes('employment');
  const domainCriticalityBoost = investorSignal || employmentSignal ? 0.25 : entities.high ? 0.2 : 0;

  const high_score =
    high_kw_ratio * 0.55 + Math.max(0, high_domain_sim) * 0.2 + domainCriticalityBoost;
  const low_score = low_kw_ratio * 0.6 + Math.max(0, low_domain_sim) * 0.4;

  let level: StakesLevel;
  let confidence: number;

  if (investorSignal && (high_signals.length >= 1 || high_domain_sim >= 0.08)) {
    level = 'HIGH';
    confidence = Math.max(high_score, 0.72);
  } else if (employmentSignal && (high_signals.length >= 1 || high_domain_sim >= 0.08)) {
    level = 'HIGH';
    confidence = Math.max(high_score, 0.7);
  } else if (high_score > 0.4) {
...
function extractRiskyEntities(text: string): { high: boolean; types: string[] } {
  ...
  if (/\binvest(or|ors|ment|ments)?\b|portfolio|shareholder|quarterly|quarter\b|earnings|benchmark|fund|fiduciar|securit|underperform|underperformance|stakeholder|guidance|disclosure|market volatility|below expectations|allocators|limited partners?/i.test(text)) {
    risky.push('investor');
  }
  if (/\bhir(e|ing)?\b|recruit|candidate|interview|employ|cultural fit|personality|discriminat|team fit|workplace|termination|promotion|manager/i.test(text)) {
    risky.push('employment');
  }
